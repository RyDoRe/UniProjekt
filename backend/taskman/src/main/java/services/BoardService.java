	...
	public Response deleteBoard(@PathParam("id") String boardID) throws Exception {
		...
		if(resultsetTasks.first()) {
			...
			if(resultsetPageIds.first()) {
				...
				Page pagewithHihgestDisplayOrder = Collections.max(pageList, Comparator.comparing(p -> p.getDisplayOrder()));

				highestPageId = pagewithHihgestDisplayOrder.getId();
				System.out.println(highestPageId);
			}
